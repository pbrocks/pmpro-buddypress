...
/**
 *  $file, $line
 *  + <?php echo basename( $file ) . $line; ?> + ' mmkay?'
 *
 * @param  [type] $file [description]
 * @param  [type] $line [description]
 * @return [type]       [description]
 */
function where_is_this_error( $file, $line ) {
	echo basename( $file ) . $line;
	?>
	<script>
		alert('Problem seems to be here:  mmkay?');	
	</script>
	<?php
}
/**
...
function pmpro_bp_level_settings() {
	// where_is_this_error( __FILE__, __LINE__ );
	if ( isset( $_REQUEST['edit'] ) ) {
		$level_id = intval( $_REQUEST['edit'] );
		echo '<h3 style="color:salmon;text-align:center;">' . $level_id . '</h3>';
			$pmpro_bp_options = pmpro_bp_get_level_options( $level_id );
			echo '<pre>';
			print_r( $pmpro_bp_options );
			echo '</pre>';
			$pmpro_bp_group_automatic_add       = 1;
			// ( $pmpro_bp_options['pmpro_bp_group_automatic_add'] ? $pmpro_bp_options['pmpro_bp_group_automatic_add'] : '1' );
					...
					<?php if ( bp_has_groups( $groups_args ) ) { ?>
							...
							if ( count( $group_ids ) > 30 ) {
								...
								while ( bp_groups() ) {
									bp_the_group();
									?>
									<div class="clickable"><input type="checkbox" id="pmpro_bp_group_automatic_add_<?php echo esc_attr( $groups_template->group->id ); ?>" name="pmpro_bp_group_automatic_add[]" value="<?php echo esc_attr( $groups_template->group->id ); ?>" 
										<?php
							...
							<tbody>
								...
								<tr>
									...
									<td>
										<div class="checkbox_box" 
										<?php
										if ( count( $registered_member_type_objects ) > 30 ) {
											?>
											style="height: 300px; overflow: auto;"<?php } ?>>							
											<?php
											foreach ( $registered_member_type_objects as $member_type => $member_type_data ) {
												?>
												<div class="clickable">
													<input type="checkbox" id="pmpro_bp_member_type_<?php echo $member_type_data->name; ?>" name="pmpro_bp_member_types[]" value="<?php echo esc_attr( $member_type_data->name ); ?>" 
													<?php
													if ( is_array( $pmpro_bp_member_types ) && in_array( $member_type_data->name, $pmpro_bp_member_types ) ) {
														echo " checked='checked'";}
													?>
														"> <?php echo $member_type_data->labels['name']; ?>
													</div>
													<?php
											}
											?>
					...
					<select id="pmpro_bp_restrictions" name="pmpro_bp_restrictions" onchange="pmpro_updateBuddyPressTRs();">
						<option value="-1" 
							...
							if ( $pmpro_bp_restrictions == -1 ) {
								?>
							selected="selected"<?php } ?>><?php _e( 'No - Lock access to all of BuddyPress.', 'pmpro-buddypress' ); ?></option>
							<?php if ( $level_id <> 0 ) { ?>
								<option value="0" 
								<?php
								if ( ! $pmpro_bp_restrictions ) {
									?>
									selected="selected"<?php } ?>><?php _e( 'No - Use non-member user settings.', 'pmpro-buddypress' ); ?></option>
								<?php } ?>
								<option value="1" 
								<?php
								if ( $pmpro_bp_restrictions == 1 ) {
						...
						<td>
							<select name="pmpro_bp_groups_page_viewing" id="pmpro_bp_groups_page_viewing">
								...
								if ( $can_view_groups_page == 0 ) {
									echo 'selected';}
								?>
									><?php _e( 'No', 'pmpro-buddypress' ); ?></option>
									<option value= '1' 
									<?php
									if ( $can_view_groups_page == 1 ) {
										echo 'selected';}
									?>
										><?php _e( 'Yes', 'pmpro-buddypress' ); ?></option>
									</select>
