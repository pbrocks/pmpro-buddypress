...
function pmpro_bp_level_settings() {
	if ( isset( $_REQUEST['edit'] ) ) {
		...
		if ( ! empty( $pmpro_bp_options['pmpro_bp_group_automatic_add'] ) ) {
			$pmpro_bp_group_automatic_add = $pmpro_bp_options['pmpro_bp_group_automatic_add'];
		}
		if ( ! empty( $pmpro_bp_options['pmpro_bp_group_can_request_invite'] ) ) {
			$pmpro_bp_group_can_request_invite = $pmpro_bp_options['pmpro_bp_group_can_request_invite'];
		}
		if ( ! empty( $pmpro_bp_options['pmpro_bp_member_types'] ) ) {
				...
				<?php if ( bp_has_groups( $groups_args ) ) { ?>
							...
							while ( bp_groups() ) {
								...
								<div class="clickable"><input type="checkbox" id="pmpro_bp_group_automatic_add_<?php echo esc_attr( $groups_template->group->id ); ?>" name="pmpro_bp_group_automatic_add[]" value="<?php echo esc_attr( $groups_template->group->id ); ?>" 
									...
									?>
									> <?php echo $groups_template->group->name . ' (' . $groups_template->group->status . ')'; ?></div> 
									<?php
							}
							?>
			...
			<tr>
				...
				<td>
					<select id="pmpro_bp_restrictions" name="pmpro_bp_restrictions" onchange="pmpro_updateBuddyPressTRs();">
						...
						?>
						><?php _e( 'No - Lock access to all of BuddyPress.', 'pmpro-buddypress' ); ?></option>
						<?php if ( $level_id <> 0 ) { ?>
							<option value="0" 
							<?php
							...
							?>
							><?php _e( 'No - Use non-member user settings.', 'pmpro-buddypress' ); ?></option>
						<?php } ?>
						<option value="1" 
						<?php
						if ( $pmpro_bp_restrictions == 1 ) {
