 ...
 *
		$array_types = array( 'select', 'select2', 'checkbox' );
 *
 * in_array( $sub_value->type, $array_types )
 */
	...
	foreach ( $pmprorh_registration_fields as $field_location ) {
		foreach ( $field_location as $rh_field ) {
			if ( $rh_field->meta_key == $meta_key && ! empty( $rh_field->buddypress ) ) {
				...
				if ( ! empty( $x_field ) ) {
					if ( in_array( $sub_value->type, $array_types ) ) {
						xprofile_set_field_data( $x_field, $user_id, maybe_serialize( $meta_value[0] ) );
					} else {
...
// add_action( 'add_user_meta', 'pmpro_bp_update_user_meta', 10, 4 );
